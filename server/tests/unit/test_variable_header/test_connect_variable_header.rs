#[cfg(test)]
mod tests {
    use server::variable_header::connect_variable_header::{
        check_variable_header_len, get_keep_alive,
    };

    #[test]
    fn should_return_a_valid_keep_alive_lsb() {
        let vec: [u8; 10] = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03];
    ...
    fn should_return_a_valid_keep_alive_msb() {
        let vec: [u8; 10] = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x03];
        match get_keep_alive(&vec) {
    ...
    fn should_return_none() {
        let vec: [u8; 10] = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        match get_keep_alive(&vec) {
