...
use server::{
    flags::connect_flags::ConnectFlags, helper::status_code::ConnectReturnCode,
    payload::connect_payload::ConnectPayload,
};
...
fn create_payload_with_username() {
    ...
    let (connect, return_code) = ConnectPayload::init(
        &connect_flags,
        &remaining_bytes,
        connect_return_code,
        sender_user_manager,
...
fn create_payload_with_username_and_password() {
    ...
    let connect_flags = ConnectFlags::init(&flags);
    let remaining_bytes = [
        0x00, 0x02, 0x5C, 0x0B, 0x00, 0x06, 0x41, 0x4C, 0x54, 0x45, 0x47, 0x4F, 0x00, 0x03, 0x41,
        0x4C, 0x54,
    ];
    let connect_return_code = ConnectReturnCode::init();
    let sender_topic_manager = TopicManager::init();
...
fn create_payload_with_will_topic_and_message() {
    ...
    let connect_flags = ConnectFlags::init(&flags);
    let remaining_bytes = [
        0x00, 0x02, 0x5C, 0x0B, 0x00, 0x05, 0x54, 0x4F, 0x50, 0x49, 0x43, 0x07, 0x00, 0x45, 0x47,
    ...
    .unwrap();
    assert_eq!(connect.get_will_topic(), Some("TOPIC".to_owned()));
    assert_eq!(connect.get_will_message(), Some("MESSAGE".to_owned()));
    assert_eq!(return_code.apply_validations(), 0x00);
}

#[test]
fn create_complete_payload() {
    let flags: u8 = 0b11101110;
    let connect_flags = ConnectFlags::init(&flags);
    let remaining_bytes = [
        0x00, 0x02, 0x5C, 0x0B, 0x00, 0x06, 0x41, 0x4C, 0x54, 0x45, 0x47,
        0x4F, // ALTEGO will topic
        0x07, 0x00, 0x45, 0x47, 0x41, 0x53, 0x53, 0x45,
        0x4D, // will message EGASSEM en hexa, al parsearlo queda como MESSAGE
        0x05, 0x00, 0x54, 0x4F, 0x50, 0x49, 0x43, 0x00, 0x03, 0x41, 0x4C, 0x54,
    ];
    let connect_return_code = ConnectReturnCode::init();
    let sender_topic_manager = TopicManager::init();
    let sender_user_manager = UserManager::init(sender_topic_manager);

    let (connect, _) = ConnectPayload::init(
        &connect_flags,
        &remaining_bytes,
        connect_return_code,
        sender_user_manager,
    ...
    .unwrap();
    assert_eq!(connect.get_will_topic(), Some("ALTEGO".to_owned()));
    assert_eq!(connect.get_will_message(), Some("MESSAGE".to_owned()));
}
