...
impl Publish {
    pub fn init(bytes: &[u8]) -> Publish {
        ...
        Publish {
            ...
            _topic: topic,
            packet_identifier: packet_identifier[0..2].try_into().expect("slice with incorrect length"),
    ...
    pub fn send_response(&self, mut stream: &TcpStream) {
        match self.qos{
            0b00000000 =>{}
            0b00000010 => {
                let connack_response = [0x40, 0x02, self.packet_identifier[0], self.packet_identifier[1]];
